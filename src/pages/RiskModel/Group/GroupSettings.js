...
import { Button, Form, Input, Radio, Breadcrumb, message, Modal, Icon, Select } from 'antd';
...
class InsuranceGroupSetting extends Component {
  ...
  querySettingList = id => {
    const { dispatch } = this.props;
    dispatch({
      type: 'riskmodel/fetchGroupRisk',
      payload: id,
      callback: res => {
        if (res.success) {
          this.setState({
            // dutyList: res.data.dutyList,
            code: res.data.policyId,
  ...
  onSave = () => {
    ...
    if (!companyCode || !code || !name || !insuranceList.every(risk => isFundationType || !!risk.name)) {
    ...
    const payload = { insuranceList, policyId: code, groupName: name, isFundationType, id, companyCode };
    dispatch({
      type: 'riskmodel/saveGroupRisk',
      payload,
      callback: res => {
        console.log(res);
        if (res.success) {
  ...
  jumpTo = () => {
    ...
    history.push('/scenario/insurancegroup');
  };
  ...
  render() {
    ...
    const getEvent = () =>
      insuranceList.map((item, index) => (
        <GroupInsurance
          riskList={riskList}
          riskNameList={riskNameList&&riskNameList.filter((risk)=>risk.companyCode===companyCode).map(risk => `${risk.name}(${risk.code})`)}
          data={item}
      ...
      <div className={styles.settingboard}>
        <div className={styles.topLine}>
          <Breadcrumb style={{ float: 'left' }}>
            ...
            <Breadcrumb.Item>团单风控模型配置</Breadcrumb.Item>
          </Breadcrumb>
          <Button
        ...
        <div className={styles.customCard}>
          <div className={styles.titleLine}>团单概览</div>
          <div className={styles.attrList} style={{ marginBottom: isFundationType ? 25 : 0 }}>
            ...
            <div className={styles.attrItem}>
              <div className={styles.label}>团体名称：</div>
              <Input
            ...
            <div className={styles.attrItem}>
              <div className={styles.label}>团体保单号：</div>
              <Input
                className={styles.attrInput}
                value={code}
            ...
            <div className={styles.attrItem}>
              ...
              <RadioGroup
                ...
                onChange={e => this.setState({ isFundationType: e.target.value, savestatus: true })}
              >
          ...
          {!isFundationType && (
            <div>
              ...
              <div className={styles.titleLine}>团单风控模型配置</div>
              {getEvent()}
              <Button
                icon="plus"
