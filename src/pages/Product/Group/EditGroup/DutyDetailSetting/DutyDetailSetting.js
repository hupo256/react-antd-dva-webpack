...
import { Button, Checkbox, Col, Form, Input, Modal, Radio, Row, Select, message, InputNumber } from 'antd';
...
class DutyDetailSetting extends Component {
  ...
  render() {
    ...
    const { dutyState, dutyFields = {} } = this.state;
    const { dutyTitle, groupIndex, insIndex, sdlIndex, dutyList = [] } = dutyState;
    ...
    return (
      <Modal
        ...
        width={1400}
        maskClosable={false}
        onCancel={() => handleDetailCancel(dutyFields, groupIndex, insIndex, sdlIndex)}
        footer={[
          <Button
            className={styles.cusModalButton}
            key="submit"
            onClick={() => handleDetailSave(dutyFields, groupIndex, insIndex, sdlIndex)}
          >
            保存
          </Button>,
        ]}
            ...
            switch (`${item.item_model}`) {
              ...
              case 'checkbox':
                ...
                item.select_value_items.forEach(forItem => {
                  const value = item.item_type > 2 ? forItem.select_code : +forItem.select_code;
              ...
              case 'select':
                ...
                item.select_value_items.forEach(forItem => {
                  const value = item.item_type > 2 ? forItem.select_code : +forItem.select_code;
                  selectOptions.push(
                    <Option value={value} key={forItem.select_code}>
                      {forItem.select_text}
                    </Option>
                  );
                });
              ...
              const selectMulOptions = [];
                ...
                fieldItem = (
                  <Select
                    ...
                    style={{ width: '100%' }}
                    onSelect={value =>
                      this.onSelectMultipleHandle(
                        value,
                        index,
                        item.select_value_items,
                        item.item_key
                      )
                    }
                          ...
                          <Row key={`select${selectIndex}`}>
                            {selectItem.selectList.length > 0 &&
                              selectItem.selectList.map((select, xIndex) => {
                                ...
                                const selectItemLast = select.selectItemLast
                                    ...
                                    <Col span={8}>
                                      {select.selectItemLast.map((lastItem, lastIndex) => {
                                        if (!Array.isArray(lastItem)) {
                                          return (
                                            <Row
                                              key={`${lastItem.linkItem.item_label}${lastIndex}`}
                                            >
                                              <Col>
                                                <FormItem
                                                  {...formItemLayout}
                                                  label={`${lastItem.linkItem.item_label}`}
                                                >
                                                  {getFieldDecorator(
                                                    `${
                                                      lastItem.linkItem.item_key
                                                    }${xIndex}${lastIndex}`,
                                                    {
                                                      rules: [
                                                        {
